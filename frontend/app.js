...
function renderResults(results) {
  ...
  if (results.length === 0) {
    resultsDropdown.innerHTML = `
      <div class="p-6 text-center text-zinc-500">
        ...
        <a 
          href="#" onclick="openSecureWA(event, ['55','73','98153','6025'], 'Olá! Não encontrei a medida de pneu que procuro no site. Gostaria de verificar a disponibilidade.')" target="_self"
          target="_blank"
          class="mt-4 inline-flex items-center gap-2 bg-black text-brand-yellow font-extrabold text-xs py-2.5 px-6 rounded-full transition-transform duration-300 hover:scale-105 shadow-md hover:bg-zinc-900"
        >
          <i data-lucide="message-circle" class="w-4 h-4 text-brand-yellow"></i>
  ...
  results.forEach(tire => {
    ...
    const message = `Olá Central Autocar! Vi no site o pneu [Código: ${tire.code}] na medida ${tire.size}, modelo ${tire.model}${priceMsg}. Gostaria de consultar a disponibilidade para instalação.`;
    const whatsappUrl = `https://wa.me/${['55','73','98153','6025'].join('')}?text=${encodeURIComponent(message)}`;
    ...
    card.innerHTML = `
      ...
      <div class="self-end md:self-center">
        <div 
          class="inline-flex items-center justify-center gap-2 bg-black text-brand-yellow font-black text-xs uppercase tracking-wider py-2.5 px-5 rounded-full transition-all duration-300 group-hover:scale-105 shadow-md hover:bg-zinc-900"
        >
          <i data-lucide="message-circle" class="w-4 h-4 text-brand-yellow"></i>
...
function createParticles() {
  ...
  for (let i = 0; i < count; i++) {
    // Beautiful luxury colors to overlay over white: grey, soft black, and golden-yellow
    const colors = [
      'rgba(18, 18, 18, 0.08)',    // Soft Charcoal
      'rgba(229, 137, 3, 0.35)',   // Translucent Brand Orange
...
function switchTab(tabId) {
  ...
  setTimeout(() => {
    ...
    buttons.forEach(btn => {
      if (btn.element) {
        if (btn.id === tabId) {
          btn.element.className = 'text-zinc-950 transition-colors text-[9px] sm:text-[10px] md:text-xs font-black uppercase tracking-wider px-2 sm:px-2.5 md:px-4 py-1 sm:py-1.5 rounded-full bg-brand-yellow';
        } else {
...
const PARCEIROS_DATA = [
  ...
  },
  {
    ...
    address: 'Av. J. S. Pinheiro, Centro – Itabuna/BA',
    phone: '(73) 98153-6025',
    whatsappUrl: `https://wa.me/${['55','73','98153','6025'].join('')}?text=Olá! Gostaria de falar com a Central Autocenter de Itabuna.`,
...
function updateCarouselUI() {
  ...
  const right = (parceiroActiveIndex + 1) % 3;
  const left = (parceiroActiveIndex + 2) % 3;
...
function navigateCarousel(direction) {
  ...
  if (direction === 'next') {
    parceiroActiveIndex = (parceiroActiveIndex + 1) % 3;
  } else {
    parceiroActiveIndex = (parceiroActiveIndex + 2) % 3;
  }
